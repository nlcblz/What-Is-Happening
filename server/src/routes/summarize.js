...
router.post('/', async (req, res, next) => {
  try {
    ...
    // 否则对所有未摘要的趋势批量生成
    const unsummarized = trendModel.getUnsummarized(5)
    ...
    for (const item of results) {
      trendModel.updateSummary(item.id, item.summary, item.summaryZh)
    }
